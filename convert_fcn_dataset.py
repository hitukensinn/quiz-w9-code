...
def dict_to_tf_example(data, label):
    ...
    idx = (data_img[:, :, 2] * 256 + data_img[:, :, 1]) * 256 + data_img[:, :, 0]
    ...
    feature_dict = {
        ...
        'image/filename': dataset_util.bytes_feature(re.split('\/+',data)[-1].encode('utf8')),
        'image/encoded': dataset_util.bytes_feature(encoded_data),
        'image/label': dataset_util.bytes_feature(encoded_label),
        'image/format':dataset_util.bytes_feature('jpeg'.encode('utf8')),#自己写不行？？
    }
